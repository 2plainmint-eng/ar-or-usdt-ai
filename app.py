...
if st.session_state['menu'] == "kimp":
    ...
    if bq2.button("500", key="buy500"): st.session_state['buy_q'] = 500.0
    ...
    with b_col1:
        if st.button("🚀 즉시 매수"):
            cost = buy_input * up * 1.0005
            if st.session_state['cash'] >= cost:
                st.session_state['avg'] = ((st.session_state['qty'] * st.session_state['avg']) + (buy_input * up)) / (st.session_state['qty'] + buy_input)
                st.session_state['qty'] += buy_input; st.session_state['cash'] -= cost
                st.success(f"{buy_input} USDT 매수 완료!"); time.sleep(0.5); st.rerun()
            else: st.error("현금 부족")
    with b_col2:
    ...
    if sq2.button("500", key="sell500"): st.session_state['sell_q'] = 500.0
    ...
    with s_col1:
        if st.button("💸 즉시 매도"):
            if st.session_state['qty'] >= sell_input:
                revenue = sell_input * up * 0.9995
                st.session_state['cash'] += revenue; st.session_state['qty'] -= sell_input
                if st.session_state['qty'] <= 0: st.session_state['avg'] = 0
                st.success(f"{sell_input} USDT 매도 완료!"); time.sleep(0.5); st.rerun()
            else: st.error("수량 부족")
    with s_col2:
        st.markdown("<div class='full-btn'>", unsafe_allow_html=True)
        if st.button("💎 전량 정산 (EXIT)"):
            if st.session_state['qty'] > 0:
                ...
                st.session_state['cash'] += revenue; st.session_state['qty'] = 0; st.session_state['avg'] = 0
                st.success("전량 정산 완료!"); time.sleep(0.5); st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
