...
# [상황 1] 로그인이 안 된 경우 -> 오직 로그인 창만 출력 후 코드 강제 종료
if not st.session_state['auth_active']:
    ...
    with col:
        ...
        pw_input = st.text_input("오두막 열쇠 (Password)", type="password")
        if st.button("시스템 접속"):
            if pw_input == "aror737":
                st.session_state['auth_active'] = True
                st.rerun() # 성공 시 전체 새로고침
            else:
                st.error("열쇠가 맞지 않습니다.")
    st.stop() # <--- 핵심! 로그인 안 됐으면 아래 코드는 실행 자체를 안 함

# [상황 2] 로그인 성공 시 (여기서부터 대시보드 시작)
...
if up_k or ex:
    ...
    if avg_global and up_k:
        ...
        st.session_state['k_logs'].append({"시간": now, "김프": k_val})
        if len(st.session_state['k_logs']) > 30: st.session_state['k_logs'].pop(0)
        
        st.write("---")
        st.subheader("📉 최근 변화 추이")
        df = pd.DataFrame(st.session_state['k_logs'])
