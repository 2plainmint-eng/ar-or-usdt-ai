...
import requests
import pandas as pd
from datetime import datetime, timedelta, timezone # 시간 보정용 추가
import time
...
def apply_ultra_contrast_style():
    st.markdown("""
        ...
        .main-title { color: #26A17B !important; font-size: clamp(1.5rem, 6vw, 2.2rem); margin: 20px 0; }
        .stMarkdown h3 {
            color: #ffffff !important; font-size: clamp(1.2rem, 5vw, 1.6rem) !important;
            margin-top: 30px !important; border-left: 5px solid #26A17B; padding-left: 10px; text-align: left !important;
        }
...
with main_placeholder.container():
    if not st.session_state['auth']:
        ...
        with col_login:
            ...
            pw = st.text_input("오두막 열쇠 (PW)", type="password", key="pw_final_time")
            if st.button("시스템 접속", key="btn_final_time"):
                if pw == "aror737":
...
if up_k and ex:
    ...
    if avg_global:
        ...
        # 💡 [핵심] 한국 표준시(KST)로 변환 (UTC+9)
        kst = timezone(timedelta(hours=9))
        now_kst = datetime.now(kst).strftime('%H:%M:%S')
        
        # 차트 기록
        st.session_state['chart_history'].append({"시간": now_kst, "김프": k_val})
    ...
    st.caption(f"최근 갱신: {now_kst} (12초 자동 갱신 중)")
